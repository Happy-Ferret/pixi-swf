...
module Shumway.flash.geom {
	...
	export class Transform extends LegacyEntity {
		...
		constructor(displayObject: flash.display.DisplayObject) {
			super();
			if (!displayObject) {
				this._sec.throwError("ArgumentError", Errors.NullPointerError, "displayObject");
			}
		...
		set matrix3D(m: flash.geom.Matrix3D) {
			if (!(this._sec.geom.Matrix3D.axIsType(m))) {
				this._sec.throwError('TypeError', Errors.CheckTypeFailedError, m, 'flash.geom.Matrix3D');
			}
		...
		getRelativeMatrix3D(relativeTo: flash.display.DisplayObject): flash.geom.Matrix3D {
			checkNullParameter(relativeTo, "relativeTo", this._sec);
		...
		get perspectiveProjection(): flash.geom.PerspectiveProjection {
			...
			let PerspectiveProjectionClass = this._sec.geom.PerspectiveProjection;
