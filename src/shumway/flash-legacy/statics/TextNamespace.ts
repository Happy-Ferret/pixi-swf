module Shumway.flash.system {
	...
	import FontType = text.FontType;
	import FontStyle = text.FontStyle;

	export class TextNamespace extends LegacyNamespace {
		constructor() {
			super();
		...
		private _deviceFontMetrics: any;

		private _getFontMetrics(name: string, style: string) {
			return this._deviceFontMetrics[name + style] || this._deviceFontMetrics[name];
		}

		resolveFontName(name: string) {
			if (name === '_sans') {
		 ...
		 */
		registerFontSymbol(fontMapping: { name: string; style: string; id: number },
		                   loaderInfo: flash.display.LoaderInfo): void {
			let syncId = display.DisplayObject.getNextSyncID();
		...
		resolveFontSymbol(loaderInfo: flash.display.LoaderInfo, id: number, syncId: number,
		                  key: string) {
		...
		DEFAULT_FONT_SANS = 'Arial';
		DEFAULT_FONT_SERIF = 'Times New Roman';
		DEFAULT_FONT_TYPEWRITER = 'Courier New';
