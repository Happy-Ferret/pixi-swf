...
module Shumway.flash.geom {
	...
	export class Matrix3D extends LegacyEntity {
		...
		setTargetDisplayObject(object: flash.display.DisplayObject): void {
			if (this._displayObject) {
				this._sec.throwError('ArgumentError', Errors.Matrix3DRefCannontBeShared);
			}
		...
		get rawData(): any {
			let result = new Float64Array(16);
		...
		decompose(orientationStyle: string = "eulerAngles"): Float64Array {
			release || notImplemented("public flash.geom.Matrix3D::decompose");
			return null;
		}

		recompose(components: Float64Array, orientationStyle: string = "eulerAngles"): boolean {
		...
		copyRawDataFrom(vector: Float64Array, index: number /*uint*/ = 0, transpose: boolean = false): void {
			index = index >>> 0;
			transpose = !!transpose;
			let m = this._matrix;
			if (transpose) {
